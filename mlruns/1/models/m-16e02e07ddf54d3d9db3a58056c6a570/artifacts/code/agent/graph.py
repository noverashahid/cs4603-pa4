...
def load_mcp_tools(server_path: str | None = None):
    """Connect to the GIVEN MCP server over stdio and return its tools.

    Loaded ONCE at graph-build time (not per-request): the stdio MCP
    server is bundled inside the model artifact, so spawning it on every
    tool call would be slow and fragile inside the serving container.
    """
